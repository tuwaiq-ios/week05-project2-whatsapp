...
class ProfileViewController: UIViewController , UITextFieldDelegate {
  ...
  let saveButton: UIButton = {
   ...
   signOutButton.setTitle("Save", for: .normal)
    signOutButton.setTitleColor(.white, for: .normal)
    signOutButton.backgroundColor = UIColor(red: (50/255), green: (112/255), blue: (166/255), alpha: 1)
    signOutButton.layer.cornerRadius = 18
    signOutButton.layer.masksToBounds = true
    return signOutButton
  }()
  ...
  @objc func saveButtonTapped(){
    var user = userNameLabel.text ?? "_"
    if user.isEmpty {
      user = "empty"
    }
    UserDefaults.standard.set(user, forKey:"textFieldName")
    var st = userNameStatus.text ?? "_"
    if st.isEmpty {
      st = "empty"
    }
